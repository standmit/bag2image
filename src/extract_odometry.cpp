...
int main(int argc, char **argv) {
    po::variables_map vm;
    {
        ...
        if (
            ...
            std::cout << "extract_odometry [options]";
            std::cout << " " << OPT_BAG;
            std::cout << " " << OPT_TOPIC;
    ...
    assert(view.begin()->getDataType() == ros::message_traits::datatype<nav_msgs::Odometry>());
    ...
    for (rosbag::MessageInstance const instance: view) {
        const auto msg = instance.instantiate<nav_msgs::Odometry>();
        if (msg == NULL)
            continue;
        ...
        std::cout << '\t' << msg->twist.covariance[0];
        for (int i = 1; i < msg->twist.covariance.size(); ++i)
            std::cout << ',' << msg->twist.covariance[i];
        std::cout << std::endl;
    }
