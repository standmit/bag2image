...
int main(int argc, char **argv) {
    po::variables_map vm;
    {
        ...
        if (
            ...
            std::cout << "extract_geopointstamped [options]";
            std::cout << " " << OPT_BAG;
            std::cout << " " << OPT_TOPIC;
    ...
    assert(view.begin()->getDataType() == ros::message_traits::datatype<geographic_msgs::GeoPointStamped>());
    ...
    for (rosbag::MessageInstance const instance: view) {
        const auto msg = instance.instantiate<geographic_msgs::GeoPointStamped>();
        if (msg == NULL)
            continue;
        ...
        std::cout << '\t' << msg->position.latitude;
        std::cout << '\t' << msg->position.longitude;
        std::cout << '\t' << msg->position.altitude;
        std::cout << std::endl;
    }
