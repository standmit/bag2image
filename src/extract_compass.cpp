...
int main(int argc, char **argv) {
    po::variables_map vm;
    {
        ...
        if (
            ...
            std::cout << "extract_compass [options]";
            std::cout << " " << OPT_BAG;
            std::cout << " " << OPT_TOPIC;
    ...
    assert(view.begin()->getDataType() == ros::message_traits::datatype<sensor_msgs::MagneticField>());

    std::cout << "nsec";
    std::cout << "\tmag_x\tmag_y\tmag_z";
    std::cout << "\tcovariance";
    std::cout << std::endl;

    for (rosbag::MessageInstance const instance: view) {
        const auto msg = instance.instantiate<sensor_msgs::MagneticField>();
        if (msg == NULL)
            continue;
        ...
        std::cout << '\t' << msg->magnetic_field.x;
        std::cout << '\t' << msg->magnetic_field.y;
        std::cout << '\t' << msg->magnetic_field.z;
        std::cout << '\t' << msg->magnetic_field_covariance[0];
        std::cout << ',' << msg->magnetic_field_covariance[1];
        std::cout << ',' << msg->magnetic_field_covariance[2];
        std::cout << ',' << msg->magnetic_field_covariance[3];
        std::cout << ',' << msg->magnetic_field_covariance[4];
        std::cout << ',' << msg->magnetic_field_covariance[5];
        std::cout << ',' << msg->magnetic_field_covariance[6];
        std::cout << ',' << msg->magnetic_field_covariance[7];
        std::cout << ',' << msg->magnetic_field_covariance[8];
        std::cout << std::endl;
    }
