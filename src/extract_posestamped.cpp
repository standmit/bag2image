...
int main(int argc, char **argv) {
    po::variables_map vm;
    {
        ...
        if (
            ...
            std::cout << "extract_posestamped [options]";
            std::cout << " " << OPT_BAG;
            std::cout << " " << OPT_TOPIC;
    ...
    assert(view.begin()->getDataType() == ros::message_traits::datatype<geometry_msgs::PoseStamped>());
    ...
    for (rosbag::MessageInstance const instance: view) {
        const auto msg = instance.instantiate<geometry_msgs::PoseStamped>();
        if (msg == NULL)
            continue;
        ...
        std::cout << '\t' << msg->pose.position.y;
        std::cout << '\t' << msg->pose.position.z;
        std::cout << '\t' << msg->pose.orientation.x;
        std::cout << '\t' << msg->pose.orientation.y;
        std::cout << '\t' << msg->pose.orientation.z;
        std::cout << '\t' << msg->pose.orientation.w;
        std::cout << std::endl;
    }
